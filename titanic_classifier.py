...
####################################################################################
####################################################################################
def titanic_random_forest(path_model_out="") :

    config_name  = 'titanic_random_forest'
    data_name    = "titanic"         ### in data/input/
    model_class  = 'RandomForestClassifier'  ### ACTUAL Class name for model_sklearn.py
    n_sample     = 1000
...
def run_all():
    data_profile(path_data_train="data/input/titanic/train", path_model="data/output/titanic")
    preprocess(config='titanic_random_forest')
    train(config='titanic_random_forest')
    check()
    predict(config='titanic_random_forest')
